...
Оновлення r8, 2026-09-06: до перевіреного r7 додано узгоджений опис PWA та актуальний текст правил без старого повідомлення «вхід недоступний». 75/75 тестів. Production повторно опубліковано й звірено по 21 asset-хешу; власник успішно ввів новий код, прихований профіль збережений і відновлений після перезавантаження. Див. [звіт про виправлення](LOGIN_FIX_2026-09-05.uk.md) та [інструкцію перенесення](CHATGPT_PROFILE_TRANSFER.uk.md).
...
- Виправлено браузерний виклик native fetch в обох адаптерах; **75/75 локальних тестів PASS**. На живому сайті форма показує «Приватний пілот», відновлює власний профіль і містить вибіркове перенесення ChatGPT. [Фінальний receipt](../artifacts/chatgpt-transfer/validation.json).
- Опублікований ZIP r8: `../synera-neon-pilot-20260906-r8-root.zip`, 24 файли. SHA256 `ADEE6788D1F68C284448272B8A3E72CAB19AF7AEC4F47EEA6D7F41C38784ED82`. Попередні ZIP збережено.
- Реальний OTP-вхід власника, прийняття правил, збереження прихованого профілю та відновлення після перезавантаження перевірені. Другий реальний акаунт, фізичні Android/iPhone та автоматичний серверний AI ще не прийняті.
...
7. `web_launch/dist-neon` — 24 файли, включно з `_worker.js`, `_routes.json` та локальним модулем вибіркового перенесення ChatGPT. SQL, тести, паролі, локальний AI й старі демоботи не потрапляють у роздачу. Worker генерує `/config.json` із серверних flags; початково все закрито.
...
## Вибіркове перенесення та AI

Користувач може скопіювати завдання з діалогу **«Мій ChatGPT»**, отримати у власному звичайному ChatGPT короткий `synera-profile-2`, вставити його в Synera та перевірити всі поля й умови. Synera не читає пам’ять чи історію ChatGPT, не приймає пароль або cookie, не робить повний архів і не вмикає видимість профілю під час імпорту. Подробиці та офіційні межі — у [CHATGPT_PROFILE_TRANSFER.uk.md](CHATGPT_PROFILE_TRANSFER.uk.md).
