...
test('HTTP: real-user assets load; legacy demo, lab, server code, private files and writes stay inaccessible', async t => {
  ...
  for (const name of ['.git/config', 'config.public.json', 'matching.test.mjs', 'assets.mjs', '../supabase/schema.sql', 'simulation.mjs', 'data.mjs', 'lab.html', 'lab-fixtures.mjs', 'server/local-profile-ai.mjs']) assert.equal((await fetch(`${url}/${name}`)).status, 404, name);
